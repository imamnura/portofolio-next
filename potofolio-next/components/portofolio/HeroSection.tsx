...
const SOCIAL_LINKS = [
  {
    href: "https://github.com/imamnura",
    icon: "simple-icons:github",
    label: "GitHub",
    color: "hover:text-gray-900 dark:hover:text-white",
  },
  {
    href: "https://www.linkedin.com/in/imamnura21/",
  ...
  return (
    <div className="relative min-h-[100dvh] flex items-center justify-center">
      ...
      >
        ...
        <motion.p
          className="text-lg md:text-xl text-gray-600 dark:text-gray-500 mb-10 max-w-2xl mx-auto leading-relaxed"
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 0.8, duration: 0.6 }}
        >
          Building immersive web experiences with clean, maintainable code and
          ...
          {SOCIAL_LINKS.map(({ href, icon, label, color }) => (
            ...
            >
              <Link
                href={href}
                target="_blank"
                className={`inline-flex items-center gap-2 px-4 py-2 bg-white/80 hover:bg-white dark:bg-gray-800/80 dark:hover:bg-gray-800 backdrop-blur-sm rounded-full text-sm transition-all duration-300 border border-gray-300/50 dark:border-gray-600/50 shadow-lg hover:shadow-xl ${color}`}
              >
                <Icon icon={icon} width={18} height={18} className="text-xl" />
                {label}
              </Link>
            </motion.div>
          ))}
